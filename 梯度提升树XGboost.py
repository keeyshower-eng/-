...
class TFIDF_XGBoost_SpamClassifier:
    """基于TF-IDF和XGBoost的垃圾邮件分类器"""
    ...
    def perform_grid_search(self, X_train, y_train, cv=3):
        ...
        param_grid = {
            ...
            'xgb__scale_pos_weight': [1, (len(y_train) - sum(y_train)) / sum(y_train)]  # 处理不平衡数据
        }
        ...
        print(f"最佳交叉验证F1分数: {self.best_score_:.4f}")
    ...
    def evaluate_model(self, X_test, y_test, threshold=None):
        ...
        # 使用阈值进行预测
        y_pred, confidence, spam_proba = self.predict_with_threshold(X_test, threshold)
        ...
        f_beta = fbeta_score(y_test, y_pred, beta=0.3, zero_division=0)
...
def main():
    ...
    print("="*60)
    print("TF-IDF + XGBoost垃圾邮件检测系统 - F0.3分数优化")
    print("="*60)
    
    # 1. 初始化分类器
    classifier = TFIDF_XGBoost_SpamClassifier(random_state=RANDOM_STATE)
    ...
    # 6. 使用最佳阈值评估模型
    print("\n使用最佳阈值评估模型性能:")
    evaluation_results = classifier.evaluate_model(X_test, y_test, best_result['threshold'])
    ...
    return classifier, best_result, all_results, evaluation_results

# 运行主程序
if __name__ == "__main__":
    try:
        classifier, best_result, all_results, evaluation_results = main()
    except FileNotFoundError:
